...
import { Settings, X, Pause, Play, ZoomIn, ZoomOut, Download, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react';
...
export const ChartCard: React.FC<{ id: string; title: string; yMin?: number | 'auto'; yMax?: number | 'auto';
...
= ({ id, title, yMin, yMax, series }) => {
  ...
  return (
    <div ref={drop as any} className="card" style={{ outline: isOver ? '1px dashed rgba(59,130,246,0.6)' : 'none' }}>
      <div className="card-title">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span className="status-badge" style={{
            ...
            borderColor: chart?.paused ? 'rgba(234,179,8,0.35)' : 'rgba(34,197,94,0.35)'
        ...
        <div className="card-actions">
          ...
          <button className="icon-button" title="Export CSV" onClick={() => {
            ...
            const csv = [cols.join(',')].concat(
              data.map(row => cols.map(c => (row as any)[c] ?? '').join(','))
            ).join('\n');
          ...
          <button className="icon-button icon-button--danger" title="Remove chart" onClick={async () => {
            const ok = await confirmBox({ message: `Delete chart "${chart?.title || 'Untitled'}"?`, confirmLabel: 'Delete', cancelLabel: 'Cancel', destructive: true });
            if (ok) removeChart(id);
          }}><X size={16} /></button>
        </div>
      </div>
      {showCfg && (
      ...
      {/* Chart area */}
      <div style={{ height: 300, position: 'relative' }} ref={containerRef as any} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp}>
        <div className="drag-zoom-overlay" style={overlayStyle} />
